	  ...
	  <div class="main-content">
			...
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title">备件信息查询</h3>
					...
					<table id="example-1" class="table table-striped table-bordered" cellspacing="0" width="100%">
						...
						<tbody>
							<c:forEach items="${data}" var="u">
								<tr>
									<td>${u.sparepartNo}</td>
									<td>${u.sparepartName}</td>
									<td>${u.sparepartType}</td>
									<td>${u.sparepartPrice}</td>
									<td>${u.sparepartTotal}</td>
									<td>${u.sparepartWarn}</td>
									<td>
										<c:choose>
									      <c:when test="${u.sparepartState=='1'}">库存正常</c:when>
									      <c:when test="${u.sparepartState=='2'}">库存临界</c:when>
									      <c:when test="${u.sparepartState=='3'}">库存紧张</c:when>
									      <c:when test="${u.sparepartState=='4'}">缺货</c:when>
									    </c:choose>
									</td>
									<td><a href="/RepairManagement/page/SparepartDeleteServlet?id=${u.sparepartNo}">删除</a></td>
							</tr>
							</c:forEach>
						</tbody>
					</table>	
